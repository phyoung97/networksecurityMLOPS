...
def _safe_load_numpy(path: str):
    try:
        from networksecurity.utils.main_utils.utils import load_numpy_array
        return load_numpy_array(path)
    except Exception as e:
        # Wrap with your project exception so the caller gets a clean error,
        # and importantly, we STOP before referencing undefined variables.
        raise NetworkSecurityException(
            Exception(f"Failed to load NumPy array: {path} -> {e}"), sys
        )
...
class ModelTrainer:
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        ...
        ## to get the best model score from our dictionary
    ...
    def initiant_model_trainer(self):
        try:
            ...
            train_arr = _safe_load_numpy(train_path)
            test_arr  = _safe_load_numpy(test_path)

            if not isinstance(train_arr, np.ndarray) or not isinstance(test_arr, np.ndarray):
                raise ValueError("train_arr/test_arr must be numpy arrays")
            if train_arr.ndim != 2 or test_arr.ndim != 2:
                raise ValueError(f"Expected 2D arrays, got train_arr.ndim={train_arr.ndim}, test_arr.ndim={test_arr.ndim}")

            # Split features/target (assumes last column is the target)
            X_train, y_train = train_arr[:, :-1], train_arr[:, -1]
            X_test,  y_test  = test_arr[:, :-1],  test_arr[:, -1]

            # Delegate to the classification trainer
